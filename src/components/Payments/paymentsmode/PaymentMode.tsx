...
function PaymentMode() {
  ...
  const cardDetailsRef: any = useRef(null);
  const addAddressRef: any = useRef(null);
  ...
  const handleOverlayClick = (event: any) => {
    if (
      ...
      !cardDetailsRef.current.contains(event.target)
    ) {
      setShowCardDetails(false);
    }
    if (
      addAddressRef.current &&
      !addAddressRef.current.contains(event.target)
    ) {
      setShowAddAddress(false);
    }
  };
