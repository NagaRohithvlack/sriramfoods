...
function CategoriesPage () {
  ...
  <div className="grid grid-cols-2 md:grid md:grid-cols-4 justify-center gap-4 md:gap-4 w-full max-w-[900px]">
    {CategoriesData.map((item) => (
      <div key={item.id} className="w-full p-8 md:p-4 flex flex-col justify-center items-center gap-4" style={{ backgroundColor: item.color, borderRadius: '130px 10px 10px 10px' }} > 
        <div className={`border-4 rounded-full overflow-hidden`} style={{ borderColor: item.bgcolor }}>
          <img src={item.img} className={`object-cover object-relative w-16 h-16 md:w-36 md:h-36`} alt={item.title} />
        </div>
        <h1 className="text-md font-normal md:font-normal md:text-3xl font-light">{item.title}</h1>
        <button onClick={() => {navigate(`${item.route}`)}} className="border border-[#4A2E1F] md:p-1 p-2 md:px-4 rounded-full">View All</button>
      </div>
    ))}
  </div>
</div>
